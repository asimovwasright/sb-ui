...
Same as shedule.php, except it should also be able to upload files, and schedule single files to be added to queue at specific time per day.
Should also be able to handle single events only, not recurring.
 ...
 <tr>
  <td>
	<?php
	  ...
	  foreach(glob($dir."jingleMonday*") as $file) {
  ...
  <td>
	<?php
	  $dir = '/var/www/html/elements/';
	  foreach(glob($dir."jingleTuesday*") as $file) {
	    echo file_get_contents($file);
	  }
	?>
  </td>
  <td>
	<?php
	  $dir = '/var/www/html/elements/';
	  foreach(glob($dir."jingleWednesday*") as $file) {
	    echo file_get_contents($file);
	  }
	?>
  </td>
  <td>
	<?php
	  $dir = '/var/www/html/elements/';
	  foreach(glob($dir."jingleThursday*") as $file) {
	    echo file_get_contents($file);
	  }
	?>
  </td>
  <td>
	<?php
	  $dir = '/var/www/html/elements/';
	  foreach(glob($dir."jingleFriday*") as $file) {
	    echo file_get_contents($file);
	  }
	?>
  </td>
  <td>
	<?php
	  $dir = '/var/www/html/elements/';
	  foreach(glob($dir."jingleSaturday*") as $file) {
	    echo file_get_contents($file);
	  }
	?>
  </td>
  <td>
	<?php
	  $dir = '/var/www/html/elements/';
	  foreach(glob($dir."jingleSunday*") as $file) {
	    echo file_get_contents($file);
	  }
	?>
  </td>
...
<div id="openUpload" onclick="popupUpload('up')">&#8613;</div><div id="openScheduleAdd" onclick="popupSchedule('up')">+</div>
...
<form method="post" action="jingleChange.php">
<br /><br />
...
<div id="scheduleChannel">
 <div id="channelSelectDrop">
  <select name="jingle" required>
      <option value="" readonly disabled selected>Jingle ?</option>
 	<?php
	echo file_get_contents( "jinglesSelect.txt" );
	?>
  </select>
 </div>
</div>
<div>
<fieldset id="startTime"><legend>@</legend>
...
<drag id="popupUpload">
  <div id="closeScheduleAdd" onclick="popupUpload('down')">&#215;</div>
  <form action="jingleUpload.php" method="post" enctype="multipart/form-data">
      Upload a Jingle: <br /><br />
...
function popupUpload(direction)
{
  var popbox = document.getElementById('popupUpload');
  if ( direction == "up" )
  {
    popbox.style.display = 'block';
  } else
